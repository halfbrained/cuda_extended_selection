...
class Command:
    ...
    def on_click_dbl(self, ed_self, state):
      ...
      if state == 'sL'  or  state == 'Ls': # only shift and left mouse are pressed 
        ...
        caret_x, caret_y = ed_self.get_carets()[0][:2]
      
        l = 0
        textl = ed_self.get_text_substr( max(0, caret_x-255), caret_y,   caret_x, caret_y )
        lmatch = re.search(incl_pattern, textl[::-1])
        if lmatch:
          l = lmatch.end()

        r = 0
        textr = ed_self.get_text_substr(caret_x, caret_y, caret_x+255, caret_y)
        if comment_str:
    ...
    def on_caret(self, ed_self):
      ...
      if self.to_sel:
        ...
        ed_self.set_sel_rect(x1, y1,  x2, y2)

      elif self.triple_sel  and  self.triple_sel[0] >= time.time():
